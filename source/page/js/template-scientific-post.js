...
let searchArea = document.querySelector('div.custom-search-box');
let searchIcon = searchArea.querySelector('div.open-search.menu-item a');
let searchBox = searchArea.querySelector('div.search-form-container');
...
let menuButton = document.querySelector('div.custom-table-of-content span.gutbcontent-menu-icon');
let stickyMenu = document.querySelector('div.custom-table-of-content div.custom-toc-wrapper');
...
jQuery(function($){
 ...
 /*  $(document).on('click', 'div.open-search.menu-item a', function(e){
  ...
  // 2. Navigation menu
  let gutbcontentMainMenu = $('ul.gutbcontent-main-menu-wrapper');
  
  let menuItems = gutbcontentMainMenu.find('li.menu-item');
  
  // traverse through tree menu - with pre-order algorithm
  for(let i = 0; i < menuItems.length; i++){
    // Access current menu item by menuItems[i] / $(menuItems[i])
    
    let hasSubMenu = $(menuItems[i]).hasClass('menu-item-has-children');    

    if( true === hasSubMenu ){      
      // 1. Calculate menu item level  
      let itemClasses = menuItems[i].classList;
      ...
      if( itemLevel == 0 ){
        ...
        let menuItemWidth = $(menuItems[i]).outerWidth(true);
        let subMenuWidth = $(menuItems[i]).children('ul').outerWidth(true);
        let displayWidth = (Math.abs(subMenuWidth) >= Math.abs(menuItemWidth)) ? Math.abs(subMenuWidth) : Math.abs(menuItemWidth);
