...
/**
 * WP Bootstrap Nav Menu Walker - WP Bootstrap Navwalker - referential package info.
 * 2021-Aug - updated to fit Bootstrap 5
 *
...
class NavigationMenuWalker extends Walker_Nav_Menu {
     ...
     * 2021-Sep : modified constructor to receive multiple arguments
     * - Reproduce' 'polymorphism behaviors of Java constructor
     * @since 4.2.0
     */
    public function __construct()
    {

      // Select sub constructors
      $arguments_list = func_get_args();
      $numberOfArguments = func_num_args();

      if (method_exists($this, $function = '__construct'.$numberOfArguments)) {
        call_user_func_array(array($this, $function), $arguments_list);
      }
      ...
      // Custom properties to be defined
      //$this->createdMenuItem = array();
      // $this->createdMenuItemLiA = array();
      $this->createdMenuItemLi = array();

      // Default current Indexed Item with 3 levels Menu
      /*$this->currentIndexedItem = array(
        'level-0' => 0,
        'level-1' => 0,
        'level-2' => 0,
      );*/
      ...
      // $this->bsCustomNavMenuDebugger = new Custom_Template_Debugger();
    }

    public function __construct1(int $depth){

      $depth = $depth < 0 ? 0 : $depth;   // display 1st level item by default
      ...
      // Build dynamic argument here.
      $this->currentIndexedItem = array();
      $level = 0;
      while($level <= $depth){
        $this->currentIndexedItem['level-'.$level] = 0;
        $level++;
      }

      // $this->currentIndexedItem = array(implode(',', $currentIndexedItemArgs));
      //$this->currentIndexedItem = $currentIndexedItemArgs;
    ...
    /*public static function construct_with_counter_item(int $depth = 0){
    ...
    public function start_lvl(&$output, $depth = 0, $args = null)
    {
      ...
      // $classes = array( 'dropdown-menu', 'ms-auto' ); // not change
      /** 2021-Aug add drop right class if $depth > 0
       * initial value: $classes = array('dropdown-menu');
       **/
      $classes = $depth > 0 ? array('dropdown-menu', 'dropend') : array('dropdown-menu');
      ...
      /*// 2021-Aug add relative position
      $inlineCss = 'style=position:relative;top:0;min-width:auto;';

      $output .= "{$n}{$indent}<ul$class_names $labelledby $inlineCss>{$n}";*/
     ...
     *
     * Problem with having sub-item : href ???
     *
     */
    public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
    {
      ...
      // $classes[] = $this->has_children ? 'dropdown-submenu' : 'dropdown-submenu-end';
      $menuItemLvl = $depth;  // menu item start with level 0
      $classes[] = $this->has_children ? 'dropdown-submenu menu-item-lvl-'.$menuItemLvl : 'dropdown-submenu-end menu-item-lvl-'.$menuItemLvl;
      // 2021-Aug - Check for diferent li level:
      /*
      if ($this->has_children && 0 === $depth) {
        $classes[] = 'dropdown-submenu menu-item-lvl-'.$menuItemLvl;
      }
      else if ($this->has_children && $depth > 0 ){
        $classes[] = 'dropdown-submenu menu-item-lvl-'.$menuItemLvl;
      }
      else {
        $classes[] = 'dropdown-submenu-end menu-item-lvl-'.$menuItemLvl;
      }*/
      ...
      // 2021-Sep add custom navigation menu item with order & depth (menu-item-2-1-9)
      $this->counterItem++;
      ...
      $currentMenuItemLi = array(
        ...
        'depth'                     => $depth,
        'registered-classnames'     => (function($arg) { return $arg->classes ?? [];})($item),
        'heading-index'             => $this->currentIndexedItem,
      );

      $this->createdMenuItemLi[] = $currentMenuItemLi;

      // Update index, get class name here
      // $classes[] = $this->get_custom_classname_li_menu_item();  // OK
      ...
      // For debug
      /*$this->createdMenuItemLi[] = $indent . '<li ' . $id . $class_names . '>';*/

      /*
       * // too much info
      $this->createdMenuItemLi[] = array(
        'DEBUG-output-variable' => $indent . '<li ' . $id . $class_names . '>',
        'DEBUG-depth' =>  $depth,
        'DEBUG-item'  =>  $item,
        'DEBUG-args'  => $args,
        'DEBUG-id'      => $id
      ...
      /**
       * 2021-Aug original value:
       *
       * If the item has_children add atts to <a>.
       *
       * if ($this->has_children && 0 === $depth)
       *
       * Original value: $atts['data-toggle']   = 'dropdown';
       **/

      // 2021-Aug-26: Display for 3 levels menu.
      if ($this->has_children && 0 === $depth) {
        // $atts['href'] = '#';
        $atts['data-bs-toggle'] = 'dropdown';
        $atts['aria-haspopup'] = 'true';
        $atts['aria-expanded'] = 'false';
        $atts['class'] = 'dropdown-toggle nav-link menu-item-lvl-'.$menuItemLvl;
        ...
        // 2021-Aug move outside
        // $atts['href'] = !empty($item->url) ? $item->url : '#';
        // For items in dropdowns use .dropdown-item instead of .nav-link.
        if ($depth > 0) {
          $atts['class'] = 'dropdown-item menu-item-lvl-'.$menuItemLvl;
        } else {
          $atts['class'] = 'nav-link menu-item-lvl-'.$menuItemLvl;
        }
      }
      ...
      // For debug
      // $this->createdMenuItemLiA[] = apply_filters('walker_nav_menu_start_el', $item_output, $item, $depth, $args);
    }
